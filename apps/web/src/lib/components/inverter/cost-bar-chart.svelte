<script lang="ts">
	...
	// One bar of total cost per period. Mirrors the server's CostSeriesPoint
	...
	// Bars are colored by sign: a credit (net < 0, earnings beat cost) reads green,
	// a cost reads primary. Driven by the chart's ordinal `c` scale (cRange
	// auto-builds it) so each bar picks its own fill.
	const COST_COLOR = 'var(--color-primary)';
	const CREDIT_COLOR = '#10b981'; // emerald-500, matches the byDay list below
	const config: Chart.ChartConfig = { net: { label: 'Total cost', color: COST_COLOR } };
