...
function profilesTemplate(profile: InitOptions["profile"]): string {
  ...
  return [
    ...
    ``,
    `// Define your inverter profile as code. Picking a role forces you to supply`,
    `// exactly what that role needs, or this won't compile. Run 'bun run build' to`,
    `// emit an installable repo (index.json + profiles/*.json), then commit + push.`,
    `export const ${ident} = defineProfile({`,
...
function readmeTemplate(packageName: string, repoName: string): string {
  return [
    ...
    "```",
    ``,
    `Edit \`src/profiles.ts\` — every profile you export is picked up. \`bun run build\``,
